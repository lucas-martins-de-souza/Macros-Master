    ...
    <div className="p-4 max-w-xl mx-auto space-y-4">
      ...
      {result && (
        <div className="space-y-4 mt-6">
          ...
          <div>
            <h3 className="text-lg font-semibold mt-4">Equivalentes em proteína por refeição (~{(result.protein / 4).toFixed(0)}g):</h3>
            <div className="space-y-2">
              {alimentosProteicos.map((item, i) => (
                <div key={i} className={`border p-2 rounded ${corClasse[item.tipo]}`}>
                  <strong>{item.nome}</strong>: {calcularEquivalente(item.gramas, result.protein / 4, 48)}g
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            <h3 className="text-lg font-semibold mt-4">Equivalentes em carboidrato por refeição (~{(result.carbs / 4).toFixed(0)}g):</h3>
            <div className="space-y-2">
              {alimentosCarboidratos.map((item, i) => (
                <div key={i} className={`border p-2 rounded ${corClasse[item.tipo]}`}>
                  <strong>{item.nome}</strong>: {calcularEquivalente(item.gramas, result.carbs / 4, 55)}g
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            <h3 className="text-lg font-semibold mt-4">Equivalentes em gordura por refeição (~{(result.fat / 4).toFixed(0)}g):</h3>
            <div className="space-y-2">
              {alimentosGorduras.map((item, i) => (
                <div key={i} className={`border p-2 rounded ${corClasse[item.tipo]}`}>
                  <strong>{item.nome}</strong>: {calcularEquivalente(item.gramas, result.fat / 4, 25)}g
                </div>
              ))}
            </div>
          </div>
